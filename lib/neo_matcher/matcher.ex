defmodule Garuda.NeoMatcher.Matcher do
  ...
  defp handle_join_or_create(%{"match_id" => ""} = match_details) do
    ...
    %{"room_name" => room_name, "player_id" => player_id, "match_id" => _match_id,
     "max_players" => max_players} = match_details
    case get_available_public_rooms(room_name) do
      [] ->
        ...
        :ets.insert(:neo_matcher, {unique_room_name, %{"players" => [player_id], "locked" => is_room_lock,
          "max_players" => max_players}})
        IO.puts("NEW ROOM CREATED => #{unique_room_name}")
        new_match_id

      room_list -> [room_name | _t] = List.first(room_list)
      ...
      :ets.insert(:neo_matcher, {room_name, %{"players" => updated_player_list, "locked" => is_room_lock,
  ...
  defp handle_join_or_create(match_details) do
    ...
    %{"room_name" => room_name, "player_id" => player_id, "match_id" => match_id,
    "max_players" => max_players} = match_details
    case get_available_private_room(room_name, match_id) do
      [] ->
      unique_room_name = room_name <> ":" <> match_id
      is_room_lock = max_players === 1
      :ets.insert(:neo_matcher_private, {unique_room_name, %{"players" => [player_id], "locked" => is_room_lock,
        "max_players" => max_players}})
      ...
      [{room_name, details} | _t] ->
        ...
        updated_player_list = [player_id | details["players"]]
        is_room_lock = Enum.count(updated_player_list) === details["max_players"]
        :ets.insert(:neo_matcher_private, {room_name, %{"players" => updated_player_list, "locked" => is_room_lock,
          "max_players" => details["max_players"]}})
        room_name |> String.split(":") |> List.last()
    end
  end
  ...
  defp get_available_public_rooms(room_name) do
    ...
    Enum.filter(unlocked_rooms, fn [room | _t] -> String.split(room, ":") |> List.first
     === room_name end)
  end
  ...
  defp manage_deletion(room_name, details, player_id) do
    ...
    with true <- index !== nil,
         false <- manage_if_lastplayer(room_name, Enum.count(details["players"]))
    do
        {_pop, new_list} = List.pop_at(details["players"], index)
        :ets.insert(:neo_matcher, {room_name, %{"players" => new_list, "locked" => false,
        "max_players" => details["max_players"]}})
    end
  end
